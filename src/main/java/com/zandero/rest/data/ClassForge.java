...
public class ClassForge {
    ...
    protected HttpResponseWriter<?> getResponseWriter(Class<?> returnType,
        ...
        try {
            ...
            if (accept != null) {
                writer = writers.get(returnType, definition.getWriter(), injection, routeContext, new MediaType[]{accept});
            }

            if (writer == null) {
                writer = writers.get(returnType, definition.getWriter(), injection, routeContext, definition.getProduces());
            }
